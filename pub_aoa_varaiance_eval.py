...
fig_height =fig_width*golden_mean*4       # height in inches
...
txt_x = 0.65
txt_y = 0.75
axs[2].set_title("Low RF activity",fontsize=8)
axs[2].set_ylabel('Probability')
axs[2].hist(ref_err, bins=n_bins, weights=np.ones_like(ref_err)/(len(ref_err)),range=[-10,10],zorder=3)
axs[2].set_xlim([-8,8])
axs[2].set_ylim([0.0,0.6])
axs[2].text(txt_x, txt_y,("$\sigma = %1.2f\degree$" % np.std(ref_err)), transform=axs[2].transAxes, fontsize=8, verticalalignment='bottom', bbox=props)
axs[2].set_xlabel('AoA error [$\degree$] \n c)')
axs[2].yaxis.set_major_locator(MaxNLocator(4)) 
axs[2].xaxis.set_major_locator(MaxNLocator(6)) 
axs[2].grid(zorder=0)

axs[3].set_title("Low RF activity with movement",fontsize=8)
axs[3].hist(movement_err, bins=n_bins, weights=np.ones_like(movement_err)/(len(movement_err)),range=[-10,10],zorder=3)
axs[3].set_xlim([-8,8])
axs[3].set_ylim([0.0,0.04])
axs[3].set_ylabel('Probability')
axs[3].text(txt_x, txt_y,("$\sigma = %1.2f\degree$" % np.std(movement_err)), transform=axs[3].transAxes, fontsize=8, verticalalignment='bottom', bbox=props)
axs[3].set_xlabel('AoA error [$\degree$] \n d)')
axs[3].yaxis.set_major_locator(MaxNLocator(4)) 
axs[3].xaxis.set_major_locator(MaxNLocator(6)) 
axs[3].grid(zorder=0)


axs[0].set_title("High RF activity",fontsize=8)
axs[0].set_ylabel('Probability')
axs[0].set_xlabel('AoA error [$\degree$] \n a)')
axs[0].hist(act_no_filter_err, bins=n_bins, weights=np.ones_like(act_no_filter_err)/(len(act_no_filter_err)),zorder=3, log=True)
axs[0].text(txt_x, txt_y,("$\sigma = %1.2f\degree$" % np.std(act_no_filter_err)), transform=axs[0].transAxes, fontsize=8, verticalalignment='bottom', bbox=props)
axs[0].set_xlim([-90,90])
axs[0].set_yticks([1e-4, 1e-3, 1e-2, 1e-1])
axs[0].set_xticks([-90,-60,-30,0,30,60,90])
axs[0].grid(zorder=0)

axs[1].set_title("High RF activity with BP filter",fontsize=8)
axs[1].hist(act_filter_err, bins=n_bins, weights=np.ones_like(act_filter_err)/(len(act_filter_err)), range=[-10,10],zorder=3)
axs[1].set_xlim([-8,8])
axs[1].set_xlabel('AoA error [$\degree$] \n b)')
axs[1].set_ylim([0.0,0.2])
axs[1].set_ylabel('Probability')
axs[1].text(txt_x, txt_y,("$\sigma = %1.2f\degree$" % np.std(act_filter_err)), transform=axs[1].transAxes, fontsize=8, verticalalignment='bottom', bbox=props)
